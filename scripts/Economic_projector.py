...
try:
    ...
    class _MissingMatplotlib:
        def __init__(self, *args, **kwargs):
            raise RuntimeError(
                "matplotlib is required for the GUI. Install it into the project's venv:\n"
...
class EconomicProjectorApp:
    ...
    def setup_output_tab(self):
        ...
        # Row 0: CBO-style summary, Row 1: text panes, Row 2: toolbar, Row 3: controls, Row 4: carousel (main)
        self.output_tab.rowconfigure(0, weight=0)
        self.output_tab.rowconfigure(1, weight=1)
        self.output_tab.rowconfigure(2, weight=0)
        self.output_tab.rowconfigure(3, weight=0)
        self.output_tab.rowconfigure(4, weight=1)  # Carousel gets significant space
        ...
        # Bottom: CAROUSEL (moved here from row 4 to replace matplotlib charts)
        # The carousel will now display all chart types in a large, responsive area
        carousel_frame = ttk.Frame(paned)
        paned.add(carousel_frame, weight=3)
        carousel_frame.columnconfigure(0, weight=1)
        carousel_frame.rowconfigure(0, weight=1)

        # Store original figure reference for backward compatibility
        # but DO NOT display it - all charts now go through carousel
        ...
        # Create a hidden canvas for matplotlib figures (used for on-demand rendering)
        # but don't add it to the display
        self.matplotlib_canvas = None
        self.matplotlib_toolbar = None

        # The carousel will be added in the interactive charts section below
        self.chart_carousel = None

        # Set up the legend interaction handler for matplotlib (if needed in future)
        ...
        # Don't connect yet - will only connect if matplotlib canvas is displayed
        ...
        # Don't connect - only used if matplotlib canvas is displayed

        # Move the toolbar frame up in the hierarchy
        ...
        # Add interactive chart carousel (now main display)
        try:
            interactive_frame = ttk.LabelFrame(self.output_tab, text="Chart Gallery")
            interactive_frame.grid(row=4, column=0, sticky="nsew", padx=5, pady=5)
            interactive_frame.columnconfigure(0, weight=1)
            interactive_frame.rowconfigure(0, weight=1)

            self.chart_carousel = ChartCarousel(interactive_frame)
            self.chart_carousel.grid(row=0, column=0, sticky="nsew")
        except Exception as e:
            # Fail gracefully if chart carousel cannot be created
            messagebox.showerror("Carousel Error", f"Failed to create carousel: {e}")
            self.chart_carousel = None
    ...
    def toggle_log_scale(self):
        ...
        NOTE: This now affects the matplotlib figure used in the app,
        though the primary display is through the carousel.
        """
        ...
        # Redraw matplotlib canvas if it's currently visible
        if self.matplotlib_canvas:
            self.matplotlib_canvas.draw()
            self.figure.canvas.draw()
        # If carousel is displayed, log scale won't affect the Plotly charts
        # (they have their own interactivity)
